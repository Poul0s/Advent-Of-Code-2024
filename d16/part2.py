	...
	for dir in (array([1, 0]), array([0, 1]), array([-1, 0]), array([0, -1])):
		...
		if v:
			...
			if v["time"] == None or v["time"] > cost:
				...
				v["time_dir"] = dir
				v["visited"] = False
				v["from_vertice"] = [curr_vertice]
			elif v["time"] == cost:
				v["from_vertice"].append(curr_vertice)
			elif v["time"] + 1001 >= cost:
				#omg wtf
				new_v = v.copy()
				new_v["time"] = cost
				new_v["time_dir"] = dir
				new_v["visited"] = False
				new_v["from_vertice"] = [curr_vertice]
...
while len(stack) > 0:
	curr = stack.pop()
	sit_pos.add(tuple(curr["pos"]))
	if curr["from_vertice"] != None:
		stack += curr["from_vertice"]
print(len(sit_pos))
